...
*
* 用法：
*	 CMD进入执行文件目录 ./nearest_neighbors -k 16 -thresh 50 测试PCD文件目录
*
* 解释：
*	 导入所有样本，计算测试样本和所有样本的卡方距离
*	 根据KNN算法，选最近K个样本
*	 结果进行可视化
*
*/
...
main(int argc, char** argv)
{
	...
	vfh_model histogram;
	// Load the first given user histogram (and ignore the rest).
	...
	pcl::visualization::PCLPlotter *plotter = new pcl::visualization::PCLPlotter("VFH0");

	reader.read(argv[pcd_indices.at(0)], *vfhs);
	cout << "VFH size: " << vfhs->points.size() << endl;
	plotter->addFeatureHistogram(*vfhs, 300);
	plotter->setTitle("VFH0");
	plotter->plot();

	plotter = new pcl::visualization::PCLPlotter("VFH1");
	std::string cloud_name = models.at(k_indices[0][1]).first;
	reader.read(cloud_name.c_str(), *vfhs);
	cout << "VFH size: " << vfhs->points.size() << endl;
	plotter->addFeatureHistogram(*vfhs, 300);
	plotter->setTitle("VFH1");
	plotter->plot();

	plotter = new pcl::visualization::PCLPlotter("VFH2");
	cloud_name = models.at(k_indices[0][2]).first;
	reader.read(cloud_name.c_str(), *vfhs);
	cout << "VFH size: " << vfhs->points.size() << endl;
	plotter->addFeatureHistogram(*vfhs, 300);
	plotter->setTitle("VFH2");
	plotter->plot();

	plotter = new pcl::visualization::PCLPlotter("VFH5");
	cloud_name = models.at(k_indices[0][5]).first;
	reader.read(cloud_name.c_str(), *vfhs);
	cout << "VFH size: " << vfhs->points.size() << endl;
	plotter->addFeatureHistogram(*vfhs, 300);
	plotter->setTitle("VFH5");
	plotter->plot();

	plotter = new pcl::visualization::PCLPlotter("VFH9");
	cloud_name = models.at(k_indices[0][9]).first;
	reader.read(cloud_name.c_str(), *vfhs);
	cout << "VFH size: " << vfhs->points.size() << endl;
	plotter->addFeatureHistogram(*vfhs, 300);
	plotter->setTitle("VFH9");
	plotter->plot();

	plotter = new pcl::visualization::PCLPlotter("VFH10");
	cloud_name = models.at(k_indices[0][10]).first;
	reader.read(cloud_name.c_str(), *vfhs);
	cout << "VFH size: " << vfhs->points.size() << endl;
	plotter->addFeatureHistogram(*vfhs, 300);
	plotter->setTitle("VFH10");
	plotter->plot();
